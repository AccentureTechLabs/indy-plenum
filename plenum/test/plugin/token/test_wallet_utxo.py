...
def test_wallet_get_total_amount_with_outputs_with_same_txn_seq_no(wallet, address1, address2, address3, vals1):
    wallet._update_outputs(outputs=vals1, txn_seq_no=3001)
...
def test_wallet_get_total_amount_with_outputs_with_diff_txn_seq_no(wallet, address1, address2, address3):
    ...
    #These results are valid because they are combined with the updated amounts in the previous test
...
def test_get_min_utxo_ge(wallet, address1, address2, address3):
    ...
    # These results are valid because they find the updated amounts from the previous test
    assert wallet.get_min_utxo_ge(0) == (address1.address, 3001, 1)
    assert wallet.get_min_utxo_ge(1) == (address1.address, 3001, 1)
    assert wallet.get_min_utxo_ge(1, address=address3.address) == (address3.address, 3004, 3)
    assert wallet.get_min_utxo_ge(2, address=address3.address) == (address3.address, 3004, 3)
    assert wallet.get_min_utxo_ge(3, address=address3.address) == (address3.address, 3004, 3)
    assert wallet.get_min_utxo_ge(4, address=address3.address) == (address3.address, 9010, 4)
    assert wallet.get_min_utxo_ge(5, address=address3.address) == (address3.address, 3001, 10)
    assert wallet.get_min_utxo_ge(11, address=address3.address) == (address3.address, 3005, 15)

def test_wallet_update_outputs_repeating_address_with_same_txn_seq_no(wallet, address1, address2, address3):
